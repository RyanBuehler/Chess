...
5. **M5** Batched parallel self-play + subtree reuse; profile on the 5090 against the expected CPU-bound regime; decide on the C++ move-gen swap with data in hand
   - M5 profiling result (5090 Laptop, 6x64 net, sims=200, concurrent_games=32, K=1): CUDA 95.4 games/hour, 6.6 positions/s, 1320 sims/s; CPU 99.8 games/hour, 5.8 positions/s, 1159 sims/s (CPU≈CUDA confirms GPU eval is NOT the bottleneck). cProfile top cumulative functions: _clear_virtual_loss DFS 986s/47.5%, chess.Board.copy() at leaves 347s/16.7%, neural-net forward 300s/14.5%, terminal_value+can_claim_threefold_repetition 267s/12.9%, chess.push() 145s/7.0%. Action taken in M5: the two self-inflicted hotspots were fixed in Python (dirty-node virtual-loss clearing replacing the full-tree DFS; copy-free leaf parking with park-time encoding) for **+78% throughput: 170.2 games/hour, 11.8 positions/s, 2356 sims/s** on the same config. Decision: C++ move-gen swap DEFERRED — remaining python-chess time (push, legal_moves, threefold probe) is now the dominant cost and the swap stays the expected next perf lever, but it is post-M8 backlog (re-profile first; terminal_value's threefold probe is the next candidate, possibly addressable with a cheaper repetition check before a full C++ backend).
