...
5. **M5** Batched parallel self-play + subtree reuse; profile on the 5090 against the expected CPU-bound regime; decide on the C++ move-gen swap with data in hand
   - M5 profiling result (5090 Laptop, 6x64 net, sims=200, concurrent_games=32, K=1): CUDA 95.4 games/hour, 6.6 positions/s, 1320 sims/s; CPU 99.8 games/hour, 5.8 positions/s, 1159 sims/s (CPU≈CUDA confirms GPU eval is NOT the bottleneck). cProfile top cumulative functions: _clear_virtual_loss DFS 986s/47.5%, chess.Board.copy() at leaves 347s/16.7%, neural-net forward 300s/14.5%, terminal_value+can_claim_threefold_repetition 267s/12.9%, chess.push() 145s/7.0%. Decision: PROCEED with C++ move-gen behind chess_env in M5.5/M6, rationale: tree-descent Python-chess calls (push, copy, legal_moves, threefold check) account for ~75% of wall time; a C++ backend eliminating Board.copy() per leaf and replacing the threefold-repetition probe will yield 3-5x throughput improvement without any algorithmic change; the _clear_virtual_loss DFS is also addressable by tracking dirty nodes rather than full tree walk.
